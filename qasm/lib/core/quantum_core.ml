...
let mconjtrans m =
  { mbits = m.mbits; minner = memoize2 (fun i j -> Complex.conj (m.minner j i)) }
...
let eye bits =
  { mbits = bits; minner = memoize2 (fun i j ->
    if (=) i j
...
let tMproduct m1 m2 =
  { mbits = ((+) m1.mbits m2.mbits); minner = memoize2 (fun i j ->
...
let qop_ry theta =
  { mbits = (Stdlib.Int.succ 0); minner = memoize2 (fun i j ->
    (fun fO fS n -> if n=0 then fO () else fS (n-1))
      (fun _ ->
      (fun fO fS n -> if n=0 then fO () else fS (n-1))
        (fun _ ->
...
let qop_swap2 =
  { mbits = (Stdlib.Int.succ (Stdlib.Int.succ 0)); minner = memoize2 (fun i j ->
    (fun fO fS n -> if n=0 then fO () else fS (n-1))
      (fun _ ->
      (fun fO fS n -> if n=0 then fO () else fS (n-1))
        (fun _ -> nTC (Stdlib.Int.succ 0))
