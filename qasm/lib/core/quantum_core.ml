...
let com_div x y = Complex.mul x (Complex.inv y)
...
let rec mat_rect2 bas rect _ a b =
  ...
  | Rec_mat (n0, a1, a2, a3, a4) ->
      mat_caseS_ n0 b (fun b1 b2 b3 b4 ->
          rect n0 a1 a2 a3 a4 b1 b2 b3 b4
            (mat_rect2 bas rect n0 a1 b1)
            (mat_rect2 bas rect n0 a2 b2)
            (mat_rect2 bas rect n0 a3 b3)
...
let rec mat_rect2_gen bas rect _ a b =
  ...
  | Rec_mat (n0, a1, a2, a3, a4) ->
      mat_caseS_ n0 b (fun b1 b2 b3 b4 ->
          rect n0 a1 a2 a3 a4 b1 b2 b3 b4
            (mat_rect2_gen bas rect n0 a1 b1)
            (mat_rect2_gen bas rect n0 a1 b2)
            (mat_rect2_gen bas rect n0 a1 b3)
            (mat_rect2_gen bas rect n0 a1 b4)
            (mat_rect2_gen bas rect n0 a2 b1)
            (mat_rect2_gen bas rect n0 a2 b2)
            (mat_rect2_gen bas rect n0 a2 b3)
            (mat_rect2_gen bas rect n0 a2 b4)
            (mat_rect2_gen bas rect n0 a3 b1)
            (mat_rect2_gen bas rect n0 a3 b2)
            (mat_rect2_gen bas rect n0 a3 b3)
            (mat_rect2_gen bas rect n0 a3 b4)
            (mat_rect2_gen bas rect n0 a4 b1)
            (mat_rect2_gen bas rect n0 a4 b2)
            (mat_rect2_gen bas rect n0 a4 b3)
...
let rec mat_eye n =
  (fun fO fS n -> if n = 0 then fO () else fS (n - 1))
    ...
    (fun n0 -> Rec_mat (n0, mat_eye n0, mat_0 n0, mat_0 n0, mat_eye n0))
    n
...
let rec mat_map f _ = function
  ...
  | Rec_mat (n0, a1, a2, a3, a4) ->
      Rec_mat
        (n0, mat_map f n0 a1, mat_map f n0 a2, mat_map f n0 a3, mat_map f n0 a4)
...
let mat_add n = mat_map2 Complex.add n

(** val mat_mul : int -> matrix -> matrix -> matrix **)

let mat_mul =
  mat_rect2_gen
    (fun a b -> Bas_mat (Complex.mul a b))
...
let rec tensor_product _ n a b =
  ...
  | Rec_mat (n0, m, m0, m1, m2) ->
      Rec_mat
        ( n0 + n,
          tensor_product n0 n m b,
          tensor_product n0 n m0 b,
          tensor_product n0 n m1 b,
  ...
  Rec_mat
    ( 0,
      ...
      Bas_mat
        (Complex.neg
           (rTC
              (Stdlib.sin (rdiv _UU03b8_ (float_of_int ((fun p -> 2 * p) 1)))))),
      Bas_mat
  ...
  Rec_mat
    ( 0,
      Bas_mat
        (com_iexp
           (rdiv
              (RbaseSymbolsImpl.coq_Ropp _UU03b8_)
              (float_of_int ((fun p -> 2 * p) 1)))),
      Bas_mat (rTC (float_of_int 0)),
      Bas_mat (rTC (float_of_int 0)),
...
let rec mat_single n t u =
  (fun fO fS n -> if n = 0 then fO () else fS (n - 1))
    ...
    (fun n' ->
      (fun fO fS n -> if n = 0 then fO () else fS (n - 1))
        (fun _ -> tensor_product (Stdlib.Int.succ 0) n' u (mat_eye n'))
        (fun t' ->
          tensor_product (Stdlib.Int.succ 0) n'
            (mat_eye (Stdlib.Int.succ 0))
            (mat_single n' t' u))
        t)
    n
        ...
        (fun _ ->
          (fun fO fS n -> if n = 0 then fO () else fS (n - 1))
            ...
            (fun t' ->
              mat_add
                (Stdlib.Int.succ 0 + n')
                (tensor_product (Stdlib.Int.succ 0) n' mat_proj0_base
                   (mat_eye n'))
...
let rec den_init n =
  (fun fO fS n -> if n = 0 then fO () else fS (n - 1))
    (fun _ -> mat_eye 0)
    (fun n0 -> Rec_mat (n0, den_init n0, mat_0 n0, mat_0 n0, mat_0 n0))
    n

(** val den_uop : int -> matrix -> matrix -> matrix **)

let den_uop n uop den = mat_mul n (mat_mul n uop den) (mat_conjtrans n uop)

(** val den_prob : int -> matrix -> matrix -> Complex.t **)

let den_prob n proj den = mat_trace n (mat_mul n den proj)

(** val den_prob_0 : int -> int -> matrix -> Complex.t **)

let den_prob_0 n t den = den_prob n (mat_proj0 n t) den

(** val den_prob_1 : int -> int -> matrix -> Complex.t **)

let den_prob_1 n t den = den_prob n (mat_proj1 n t) den

(** val den_measure : int -> matrix -> matrix -> matrix **)

let den_measure n proj den =
  mat_scale n
    (Complex.inv (den_prob n proj den))
    (mat_mul n (mat_mul n proj den) proj)

(** val den_measure_0 : int -> int -> matrix -> matrix **)

let den_measure_0 n t den = den_measure n (mat_proj0 n t) den

(** val den_measure_1 : int -> int -> matrix -> matrix **)

let den_measure_1 n t den = den_measure n (mat_proj1 n t) den

(** val den_reset : int -> int -> matrix -> matrix **)

let den_reset n t den =
  mat_add n
    (mat_mul n (mat_mul n (mat_proj0 n t) den) (mat_proj0 n t))
    (den_uop n
       (mat_single n t
          (mat_rot
             (4. *. Stdlib.atan 1.)
...
let mat_swap n q1 q2 =
  ...
  if s then
    ...
    if s0 then
      ...
      | None ->
          tensor_product
            (q2 + (sub q1 q2 + Stdlib.Int.succ 0))
            (sub (sub n q1) (Stdlib.Int.succ 0))
            (tensor_product q2
               (sub q1 q2 + Stdlib.Int.succ 0)
               (mat_eye q2)
...
let mat_cnot n qc qt = mat_ctrl_single n qc qt mat_not2
        ...
        in
        {
          ...
          w_qstate =
            mat_add nq
              (mat_scale nq
                 (com_div (rTC w_prob0)
                    (Complex.add (rTC w_prob0) (rTC w_prob1)))
                 w_qstate0)
              (mat_scale nq
                 (com_div (rTC w_prob1)
                    (Complex.add (rTC w_prob0) (rTC w_prob1)))
                 w_qstate1);
          w_cstate = w_cstate0;
          w_prob = RbaseSymbolsImpl.coq_Rplus w_prob0 w_prob1;
        }
        :: l
      ...
      if s then
        {
          ...
          w_qstate =
            den_uop nq
              (mat_single nq target (mat_rot theta phi lambda))
              w_qstate0;
          w_cstate = w_cstate0;
          w_prob = w_prob0;
        }
        :: execute_rotate_instr nq theta phi lambda target l
      else
        {
          w_num_clbits = w_num_clbits0;
          w_qstate = w_qstate0;
...
let rec execute_cnot_instr nq control target = function
  ...
  | w :: l ->
      ...
      if s then
        let s0 = control < nq in
        if s0 then
          let s1 = target < nq in
          if s1 then
            {
              w_num_clbits = w_num_clbits0;
              w_qstate = den_uop nq (mat_cnot nq control target) w_qstate0;
              w_cstate = w_cstate0;
              w_prob = w_prob0;
            }
            :: execute_cnot_instr nq control target l
          else
            {
              w_num_clbits = w_num_clbits0;
              w_qstate = w_qstate0;
  ...
  | w :: l ->
      ...
      if s then
        ...
        if s0 then
          {
            w_num_clbits = w_num_clbits0;
            w_qstate = den_uop nq (mat_swap nq q1 q2) w_qstate0;
            w_cstate = w_cstate0;
            w_prob = w_prob0;
          }
          :: execute_swap_instr nq q1 q2 l
        else
          {
            w_num_clbits = w_num_clbits0;
            w_qstate = w_qstate0;
...
let rec execute_measure_instr nq qbit cbit = function
  ...
  | w :: l ->
      ...
      let s = qbit < nq in
      if s then
        let prob0 = (fun x -> x.re) (den_prob_0 nq qbit w_qstate0) in
...
let rec execute_reset_instr nq target = function
  ...
  | w :: l ->
      ...
      if s then
        {
          w_num_clbits = w_num_clbits0;
          w_qstate = den_reset nq target w_qstate0;
          w_cstate = w_cstate0;
          w_prob = w_prob0;
        }
        :: execute_reset_instr nq target l
      else
        {
          w_num_clbits = w_num_clbits0;
          w_qstate = w_qstate0;
            ...
            (map
               (fun w ->
                 if eqb (w.w_cstate cbit) cond then
                   execute_suppl nq ir' subinstr (w :: [])
                 else w :: [])
               worlds)
